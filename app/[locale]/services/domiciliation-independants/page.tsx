...
import { Check, Home, ShieldCheck, Briefcase, ChevronDown } from 'lucide-react';
...
export async function generateMetadata({
    ...
    return {
        title: t('domiciliation_freelance_title'),
        description: t('domiciliation_freelance_desc'),
        keywords: t('domiciliation_freelance_keywords'),
        alternates: generateAlternates(locale, '/services/domiciliation-independants'),
    };
}

const domiciliationFreelanceServiceSchema = {
    '@context': 'https://schema.org',
    '@type': 'Service',
    name: 'Domiciliation pour indépendants à Bruxelles',
    provider: {
        '@type': 'LocalBusiness',
        name: 'Office Factory',
        address: {
    ...
    description: 'Domiciliation professionnelle pour indépendants et freelances à Bruxelles. Séparez votre adresse privée de votre activité professionnelle.',
    serviceType: 'Business Domiciliation for Freelancers',
};

export default function DomiciliationFreelancePage() {
    const t = useTranslations('DomiciliationFreelancePage');
    const tCommon = useTranslations('Common');
    return (
        <main className="min-h-screen flex flex-col bg-white">
            <script
                type="application/ld+json"
                dangerouslySetInnerHTML={{ __html: JSON.stringify(domiciliationFreelanceServiceSchema) }}
            />
            <Navbar />
            ...
            {/* Why Separate Business and Home Address */}
            ...
            <section className="py-20 bg-[#F8F9FA]">
                <div className="container">
                    <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
                        {[
                            { icon: Home, key: 'ben1' },
                            { icon: ShieldCheck, key: 'ben2' },
                            { icon: Briefcase, key: 'ben3' },
                            { icon: Check, key: 'ben4' },
                        ].map(({ icon: Icon, key }) => (
